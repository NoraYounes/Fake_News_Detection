...
def wordpre(x):
    x = x.lower()
    x = re.sub('(?<!\w)([A-Za-z])\.', r'\1', x) #Cathy's
    x = re.sub('“|’|\'|"|”', '', x) # Cathy's
    x = re.sub('\[.*?\]', '', x)
    x = re.sub("\\W"," ",x) # remove special chars
...
article_df['title_count'] = article_df['title'].apply(len)
article_df['text_count'] = article_df['text'].apply(len)
...
print(article_df.loc[[0]])
article_df.loc[[0]].to_csv('chelsea.csv')
